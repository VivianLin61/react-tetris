...
const Tetris = () => {
  ...
  const updateSideStage = (prevHoldStage, piece) => {
    const newStage = prevHoldStage.map((row) =>
  ...
  return (
    <StyledTetrisWrapper
      ...
      onKeyUp={keyUp}
    >
      <Header score={`${gameScore}`} callback={startGame} />
      <StyledTetris>
        <SideWrapper>
          {true ? (
            <>
      ...
      <ButtonsWrapper position={"center"}>
        <Button type={"Run AI"} size={"large"} callback={startAI} />
      </ButtonsWrapper>
      {/* <ButtonsWrapper position={"center"}>
        <Button type={"Train AI"} size={"large"} callback={trainAI} />
      </ButtonsWrapper> */}
    </StyledTetrisWrapper>
  );
};
